...
#include "turbulentPotentialAlphaEh.H"
...
defineTypeNameAndDebug(turbulentPotentialAlphaEh, 0);
addToRunTimeSelectionTable(RASModel, turbulentPotentialAlphaEh, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

tmp<volScalarField> turbulentPotentialAlphaEh::Ts() const
{ 
	if(tslimiter_ == "true")
	{
...
tmp<volScalarField> turbulentPotentialAlphaEh::Ls() const
{
...
turbulentPotentialAlphaEh::turbulentPotentialAlphaEh
(
...
tmp<volSymmTensorField> turbulentPotentialAlphaEh::R() const
{
...
// Not used but necessary for RAS Model
tmp<volSymmTensorField> turbulentPotentialAlphaEh::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> turbulentPotentialAlphaEh::divDevReff() const
{
    return
    (
       fvc::grad(phiReal())
...
bool turbulentPotentialAlphaEh::read()
{
    if (RASModel::read())
    {
...
void turbulentPotentialAlphaEh::correct()
{
